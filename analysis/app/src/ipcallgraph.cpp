...
void IPCallGraphNode::insertCallTarget(address_t iaddr, bool isDirect, IPCallGraphNode * t) {
    ...
    if (it != childrenMap.end()) {
        ...
        childrenMap[iaddr] = listManager.getOrCreate({
            t
        });
    }
}
    ...
    for (auto module: CIter::children(this -> program)) {
        ...
        for (auto region: CIter::regions(module)) {
            ...
            for (auto dataSection: CIter::children(region)) {
                ...
                for (auto
                    var: CIter::children(dataSection)) {
                    tuple < Module * , DataVariable * > tup1(module,
                        var);
                    ...
                    if (link) {
                        ...
                        if (auto target = dynamic_cast < Function * > ( & * link -> getTarget())) {
                            ...
                            LOG(20, "\t \t \t \t Link to DataSection " <<
                                var -> getAddress() << " : " << target -> getName() << " " << target -> getType() << " " << link -> getTargetAddress());
...
void IPCallGraph::writeCallgraphToBinaryFile() {
        ...
        for (const auto & c: node -> getDirectChildren()) {
            ...
            for (auto nn: * (c.second)) {
                ...
                edges.push_back(EdgeInfo {
                    node_map[caller_key], callsite_address, node_map[callee_key], static_cast < uint8_t > (EdgeType::Direct)
                });
            }
        }
        for (const auto & c: node -> getIndirectChildren()) {
            ...
            for (auto nn: * (c.second)) {
                ...
                if (node -> isIcallResolved(c.first)) {
                    edges.push_back(EdgeInfo {
                        node_map[caller_key], callsite_address, node_map[callee_key], static_cast < uint8_t > (EdgeType::Resolved)
                    });
                } else {
                    edges.push_back(EdgeInfo {
                        node_map[caller_key], callsite_address, node_map[callee_key], static_cast < uint8_t > (EdgeType::Indirect)
                    });
                }
            }
        }
    ...
    for (const auto & node: nodes) {
        file.write(reinterpret_cast <
            const char * > ( & node.function_address), sizeof(node.function_address));
    ...
    file.write(reinterpret_cast <
        const char * > (edges.data()), edges.size() * sizeof(EdgeInfo));
...
void IPCallGraph::writeStringToFile(std::ofstream & file,
